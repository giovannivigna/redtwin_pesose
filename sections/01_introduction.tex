...
scale~\cite{measuring}, and it is precisely this widening gap that the
...
servers, cannot flood the broker with malformed messages, cannot run an
exploit against the identity provider, and cannot even scan the device
gateways without risking a crash of the devices themselves. As a result, the
questions that matter remain unanswered. Can an attacker who compromises a
...
path that the access rules did not anticipate? Each of these questions concerns the \emph{deployment} rather than any single component, and none can be answered from an advisory or a source repository. \sysname
is designed precisely for this situation: it reconstructs the deployment as a
...
authorization decisions. A staging environment that omits the device gateways, or that seeds an empty user directory, will not exhibit the attack paths exploitable in the real infrastructure. \sysname instead
constructs a digital twin whose fidelity is measured and controlled with
respect to explicit security properties, as described in
...
Section~\ref{sec:thrust1}. The twin is therefore not a convenience for
developers, but a security artifact with quantifiable representativeness.
