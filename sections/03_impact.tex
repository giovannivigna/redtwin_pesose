...
The security of nginx and ZMap is not a property of two programs. It is a
property of everything built on top of them.
...
is running, where, and at what version. When that question is answered
incompletely, every downstream decision inherits the gap, because hosts that
are unknown are also hosts that are unpatched. Improving the safety and the
...
these specific components generates returns far out of proportion to its cost, as every
vulnerability that \sysname finds and drives upstream is removed
