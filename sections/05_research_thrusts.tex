...
finding is a false positive, or an artifact of an imprecise replication, the
information flows back to the Knowledge Base and Observer Agents, and in this
way the analysis does not merely consume the fidelity of the twin, but
continuously improves it. In the health-care deployment of
...
package, release, and build options that determine its vulnerabilities, as
knowing that a deployment runs nginx is nearly useless, whereas knowing its
version, its module set, and its directive configuration is actionable.
...
is possible to derive their security properties and dependencies. Accurately
modeling these dependencies is essential, because a dependency missed during
capture becomes an attack path that cannot be tested in the twin.

\paragraph{Safe active discovery: extending ZMap.}
Passive observation cannot resolve every question, as whether a service is
reachable from a given segment, and what version it runs, is often answerable
only by sending a packet. The stateless design of ZMap~\cite{zmap} makes it
...
non-aggressive, the resulting model is necessarily incomplete, and a central
methodological commitment of this project is to measure that incompleteness
rather than assume it away. We will therefore develop a \emph{model coverage}
metric that estimates what fraction of the infrastructure the capture has
...
the agents will actually work continuously, as new documentation will trigger
new observation plans, and observations inconsistent with the documentation
will be presented to the administrator for disambiguation. We envision this
...
modeling perspective, and a single instance can stand for all of them. This,
of course, reduces the veracity of the twin, as attacks that exploit resource
bottlenecks will not transfer. Therefore, this abstraction process is a
trade-off between the faithful replication of a system and the minimization
of resource usage, and it is a trade-off that \sysname makes explicit and
quantifiable. We will formalize this idea as an equivalence relation over the
...
primitives modeled in Thrust~3.
...
in the twin. This matters directly for our anchor product, as request
smuggling is a property of a \emph{pairing} and not of nginx alone, and a
twin that collapses distinct pairings into one loses exactly the structure
that makes those defects visible.
...
record counts and sizes, and, in the case of hospitals and other heath-focused organizations, domain objects such as DICOM images or FHIR
...
approximated, and which are lost. The measure will distinguish
\emph{soundness}, meaning that every attack path present in the original has
a corresponding path in the twin, from \emph{precision}, meaning that the
paths in the twin correspond to real paths in the original. The
equivalence-class abstraction is designed to preserve soundness while trading
away precision in a controlled way, and when Thrust~3 reports a finding that
does not reproduce, the loss of precision is localized to a specific
abstraction decision that the Replication Agents can then refine. 
...
We are explicit about the limits of this guarantee, because overstating it
would be the most consequential error this project could make. The attack
graph of the original can only be derived from the captured model, itself the
product of deliberately non-aggressive observation. Our soundness result is
therefore \emph{soundness relative to the captured model}, rather
than an unconditional claim about the physical system. On the reference
...
Agents. Mocks are a principled source of \emph{false negatives}, as a mock
reproduces behavior that was observed, whereas exploitable behavior is very
often behavior that was never observed. We therefore treat every mocked
...
will be an excellent starting point, as it demonstrated that static and
dynamic analysis combined with runtime observation can execute proprietary
firmware on emulators such as Qemu~\cite{qemu}, even without documentation
...
can compose. Here the open-source nature of the ecosystem is a key advantage,
as the source and the build configuration of each component are available,
and the agents can analyze it with a depth that would be impossible for
closed appliances.
...
actually occur in captured deployments, differentially testing how nginx and
each upstream parse the same request, a class of defect that can only be
found by testing the pair. Confirmed defects are reported upstream
under the process of Section~\ref{sec:buildtest}. Where a defect is
configuration-induced rather than a code flaw, the deliverable is detection
tooling and documentation rather than a patch.
...
allocations, frees, reads, writes, and file operations, and by modeling the
effects and the preconditions of these operations abstractly, exploitation
becomes a tractable search problem for a solver. Furthermore, this same
...
Because continuity is the central claim of this project, we quantify it
rather than assert it. \sysname operates on three cadences:
...
query against a twin that already exists. The dominant recurring costs are
LLM inference and the compute for twin instantiation and fuzzing, and
establishing the cost per cycle, and reducing it through incremental
re-capture and caching, is an explicit engineering objective that
Section~\ref{sec:evaluation} reports as a first-class metric.

\paragraph{From findings to prevention, detection, and recovery.}
Consistent with the goals of PESOSE Track~3, the output of this thrust is
oriented toward prevention, detection, and recovery rather than toward attack
for its own sake. The result of an analysis is a detailed document that describes the
multi-step attack, and this document is communicated to a human security
analyst, who will have to validate the findings on the actual system. False
positives trigger a re-evaluation of the fidelity of the replication, and in
this way Thrust~3 not only produces actionable findings, but also
continuously improves the twin. Each confirmed chain is accompanied by the
